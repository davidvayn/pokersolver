...
impl FlopSolver {
    ...
    fn frozen_walk(
        ...
        frozen_flop_walk::Walker {
            ...
            legal: &self.legal,
            nodes: &self.nodes,
            terminal: &|state, reaches| self.terminal_values(state, reaches),
            turn: &|state, reaches, traverser| self.turn_leaf_values(state, reaches, traverser),
        }.walk(state, reaches, traverser, accumulate_average, deltas)
    }
...
mod tests {
    ...
    pub(super) fn zero_value_network() -> PublicValueNetwork {
