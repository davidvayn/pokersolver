...
def combine_training_datasets(primary: Dataset, supplements: list[Dataset]) -> Dataset:
    ...
    source_policies = [
        component.source.get("source_policy_sha256") for component in components
    ]
    if any(
        not isinstance(source_policy, str) or len(source_policy) != 64
        for source_policy in source_policies
    ):
        raise ValueError("combined datasets must pin every frozen source policy")
    if any(
    ...
    # Public counterfactual values are explicitly conditioned on both exact
    # ranges, so accepted beliefs generated by older policies remain valid
    # off-policy training coverage. Tuning and holdout membership still comes
    # only from the primary (current-policy) corpus; preserve every generator
    # identity instead of pretending the supplemental beliefs are on-policy.
    source["component_source_policy_sha256"] = source_policies
    source["validation"] = {
...
def main() -> None:
    ...
    report = {
        ...
        "componentSourcePolicySha256": dataset.source.get(
            "component_source_policy_sha256",
            [dataset.source.get("source_policy_sha256")],
        ),
