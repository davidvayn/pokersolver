#!/usr/bin/env python3
"""Blend two one-step range-policy updates from the same frozen parent."""
...
def blend_policy(
    ...
    if any(len(value) != 64 for value in (source_sha256, first_sha256, second_sha256)):
        raise ValueError("hybrid update provenance hashes are invalid")
    ...
    for key in static_keys:
        if source.get(key) != first.get(key) or source.get(key) != second.get(key):
            raise ValueError(f"hybrid update component metadata differs at {key}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--component-a", type=Path, required=True)
    parser.add_argument("--component-b", type=Path, required=True)
