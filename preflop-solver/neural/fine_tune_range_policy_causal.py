...
MAXIMUM_SOURCE_PARITY_ABSOLUTE_ERROR = 0.005
...
def source_parity_metrics(
    ...
    result["accepted"] = bool(
        ...
        and result["primaryActionAgreement"] >= MINIMUM_SOURCE_PARITY_PRIMARY_AGREEMENT
...
def train_candidate(
    ...
    # Current-profile corpora were produced by exact Rust serving inference.
    # Anchor their mirror step and measured gain to those stored probabilities,
    # not to a numerically close MLX recomputation. Mirror-Prox correctors still
    # anchor to the separate frozen parent while using look-ahead action values.
    target_anchor_training = (
        training.current if same_attribution_policy else source_training
    )
    # The validation corpus is generated by the other seed. Its stored current
    # probabilities therefore belong to that other policy and cannot be the
    # candidate's frozen baseline. Cross-seed validation must always compare
    # against this candidate's source model evaluated on the validation rows.
    target_anchor_validation = source_validation
    ...
    validation_targets = mirror_descent_targets(
        target_anchor_validation,
        validation.action_values,
        validation.action_masks,
        mirror_step_per_bb,
        maximum_target_node_kl,
    )
    target_metrics = metrics(targets, training, frozen=target_anchor_training)
    validation_target_metrics = metrics(
        validation_targets, validation, frozen=target_anchor_validation
    )
    if any(
    ...
    for step_index in range(steps):
        ...
        validation_metrics = metrics(
            ...
            frozen=target_anchor_validation,
        )
        feasible = all(
    ...
    diagnostics = {
        ...
        "targetAnchorProbabilities": (
            "exact_stored_rust_training_and_mlx_cross_seed_probabilities"
            if same_attribution_policy
            else "mlx_frozen_parent_probabilities"
        ),
